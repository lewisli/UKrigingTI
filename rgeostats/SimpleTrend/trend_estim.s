...
library(MASS)
library(fields)
source('/home/lewisli/Dropbox/Shared Folders/SlideingScheme (1)/Results/Trend-OLS/RGeoS/cubic.s')
...
for (k in 1:Ns){
	...
	gammadat1 = vario.calc(dbdat)
	plot(gammadat1,npairpt=1,npairdw=TRUE,title="Residuals1", ylim=c(0,0.1))
	modeldat = model.auto(gammadat1,struct =melem.name(c(1,2)),npairpt=T,draw=T)
	print(modeldat)
	...
	gammadat2 = vario.calc(dbdat)
	plot(gammadat2,npairpt=1,npairdw=TRUE,title="Residuals2", ylim=c(0,0.05))
	modeldat = model.auto(gammadat2,struct =melem.name(c(1,2)),npairpt=T,draw=T)
	print(modeldat)
	COVdat = matrix(model.eval(modeldat,dist,as.cov=T),n)
	invCOVdat = chol2inv(chol(COVdat))
	# dual kriging
